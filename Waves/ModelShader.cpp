	...
	)
{
	...
	for (int i = 0; i != numSubModels; ++i)
	{
		...
		ID3D11ShaderResourceView *texture = material[i].texture->GetTexture();
		deviceContext->PSSetShaderResources(0, 1, &texture);

		deviceContext->DrawIndexed(
			3*(subModels[i].end - subModels[i].begin), 
			subModels[i].begin * 3, 
			0);
	}
...
bool ModelShader::InitializeShader(
	ID3D11Device* device,
	HWND hwnd,
	WCHAR* vsFilename,
	WCHAR* psFilename)
...
bool ModelShader::SetShaderParameters(
	ID3D11DeviceContext* deviceContext,
	D3DXMATRIX worldMatrix,
	D3DXMATRIX viewMatrix,
	D3DXMATRIX projectionMatrix,
...
void ModelShader::RenderShader(ID3D11DeviceContext* deviceContext)
{
