...
bool TerrainShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix,
	D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,
	ID3D11ShaderResourceView* textureFlat, ID3D11ShaderResourceView* textureSloped,
	D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor, D3DXVECTOR4 fillColor)
{
	bool result;

	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureFlat, textureSloped, lightDirection, diffuseColor, fillColor);
	if (!result) return false;
	...
	D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor, D3DXVECTOR4 fillColor)
{
	...
	deviceContext->PSSetShaderResources(0, 1, &textureFlat); //TODO: look into if there is a way to do this better than just assigning two different textures
	deviceContext->PSSetShaderResources(1, 1, &textureSloped);
